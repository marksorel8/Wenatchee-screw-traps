...
load_dat<-function(){
  ...
  return(list(chiw=chiw_dat,nas=list(nas_effic=Nas_Whi_Dat$effic_dat$Nason_ET,nas_catch=Nas_Whi_Dat$catch_ops_dat$Nason_catch),whi=list(nhi_effic=Nas_Whi_Dat$effic_dat$White_ET,whi_catch=Nas_Whi_Dat$catch_ops_dat$White_catch)))
}
...
make_ts<-function(myDates,myCounts,main){
...
out<-ts(
myCounts[match(drop_leap_day(seq.Date(min(myDates),
         max(myDates),by=1)),myDates)],start=as.numeric(c(format(min(myDates), "%Y"),format(min(myDates), "%j"))),frequency=365)
