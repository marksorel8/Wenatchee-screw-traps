...
flow_all <- renameNWISColumns(readNWISdv(siteNumbers = "13344500", parameterCd = "00060", statCd = "00003",
                                         startDate = paste(min(trap_catch_all$brood_year) + 1, "01", "01", sep = "-"),
                                         endDate = paste(max(trap_catch_all$brood_year) + 2, "12", "31", sep = "-")))
...
dates <- data.frame(brood_year = rep(unique(trap_catch_all$brood_year), 
                                     tapply(trap_catch_all$day, trap_catch_all$brood_year, max)),
                    day = unlist(sapply(tapply(trap_catch_all$day, trap_catch_all$brood_year, max),
                                        function(x) 1:x)))
row.names(dates) <- NULL
dates$week <- ceiling(dates$day/7)
dates$date <- as.Date(paste0(dates$brood_year + 1, "-09-30"), format = "%Y-%m-%d") + dates$day
flow_daily <- merge(dates, flow_all[,c("date","flow")], all.x = TRUE)
flow_weekly <- aggregate(flow ~ week + brood_year, data = flow_daily, mean)

# Stan data for single-year model
brood_year <- 2001
trap_catch <- na.omit(trap_catch_all[trap_catch_all$brood_year==brood_year,])
...
trap_catch_my$brood_year_f <- as.numeric(factor(trap_catch_my$brood_year))
MR_my <- na.omit(MR_all[is.element(MR_all$brood_year, trap_catch_all$brood_year),])
MR_my$brood_year_f <- as.numeric(factor(MR_my$brood_year))
X_p_my <- matrix(1,max(trap_catch_my$brood_year_f)*max(trap_catch_my$week),1)
X_M_my <- matrix(1,max(trap_catch_my$brood_year_f)*max(trap_catch_my$day),1)

stan_dat_my <- list(N_MR = nrow(MR_my),
                    MR_year = MR_my$brood_year_f,
                    MR_week = MR_my$week,
                    mark = MR_my$mark,
                    recap = MR_my$recap,
                    NX_p = ncol(X_p_my),
...
with(stan_dat, {
  ...
  mtext(side = 3, line = 1, paste("Tucannon brood year", brood_year))
