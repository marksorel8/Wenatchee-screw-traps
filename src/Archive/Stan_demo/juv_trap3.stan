...
parameters {
  ...
  real<lower=0> beta_NB[Use_NB];         //ispersion paramater for the Negative Binomial observation model for catch. 
  }
...
model {
 ...
 if(Use_NB){
   C ~ neg_binomial(C_hat*beta_NB[1],beta_NB[1]);
   }
 else{
...
generated quantities {
 ...
 for(t in 1:max_day){
   ...
   if(Use_NB){
   M[t] = neg_binomial_rng(M_hat[t]*beta_NB[1],beta_NB[1]);}
   else{
