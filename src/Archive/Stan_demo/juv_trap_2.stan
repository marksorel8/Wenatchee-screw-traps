...
parameters {
  ...
  real<lower=0> beta_NB[Use_NB];     // inverse square root dispersion paramater for the Negative Binomial observation model for catch. 
}
...
transformed parameters {
 ...
 // vector<lower=0>[N_trap] M_hat_cumsum;  // daily means summed over days that trap is fishing
  ...
  M_hat = exp(log_M_hat);
  if(Use_NB) alpha_NB = M_hat* beta_NB[1];
  // Expected catches
  // Note that the Poisson distribution is closed under addition
  for(i in 1:N_trap){
    alpha_NB_cumsum[i] = sum(alpha_NB[(trap_day[i] - elapsed_time[i] + 1):trap_day[i]]);
  alpha_NB_hat = alpha_NB_cumsum .* p[trap_week];
  C_hat= alpha_NB_hat ./ beta_NB[1];
}
}
...
model {
 ...
 if(Use_NB){
   C ~ neg_binomial(alpha_NB_hat,beta_NB[1]);
   }
 else{
...
generated quantities {
 ...
 for(t in 1:max_day){
   ...
   if(Use_NB){
   M[t] = neg_binomial_rng(alpha_NB_hat[t],beta_NB[1]);}
   else{
