...
for(tt in seq(0.75*parms["tau"], max(times), parms["tau"])) 
  plot(M ~ R0, data = R0Mt, subset = time == tt, type = "l", col = "darkblue", lwd = 3, 
       las = 1, cex.axis = 1.2, cex.main = 1.5, main = bquote(italic(t) == .(round(tt,2))))
mtext(bquote(italic(R)[0]), cex = 1.5*par("cex"), side = 1, line = 2, outer = TRUE)
mtext(bquote(Delta * italic(M)[italic(t)]), cex = 1.5*par("cex"), side = 2, line = 0.5, outer = TRUE)

# Plot log ratio of proportion of migrants in each pulse vs R[t=0]
# (reference class is final pulse)
dev.new(width = 9, height = 3)
par(mfrow = c(1,3), mar = c(1,2.5,2,1), oma = c(3,2,0,0))
for(tt in seq(0.75*parms["tau"], max(times) - parms["tau"], parms["tau"])) 
  plot(alrM ~ R0, data = R0Mt, subset = time == tt, type = "l", col = "darkblue", lwd = 3, 
       las = 1, cex.axis = 1.2, cex.main = 1.5, main = bquote(italic(t) == .(round(tt,2))))
mtext(bquote(italic(R)[0]), cex = 1.5*par("cex"), side = 1, line = 2, outer = TRUE)
mtext(bquote(alr(Delta * italic(M)[italic(t)])), cex = 1.5*par("cex"), side = 2, line = 0.5, outer = TRUE)
